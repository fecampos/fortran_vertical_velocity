      subroutine write_vertical_velocity(nx,ny,nz,nt,X,Y,Z,T,missing_value,wo)
      
      use netcdf 

      implicit none      

      character(len=*), parameter :: file_out="output_wo.nc"

      real :: missing_value

      integer :: nx, ny, nz, nt

      real :: X(nx), Y(ny), Z(nz), T(nt), wo(nx,ny,nz,nt)
      ...
      character(len=*), parameter :: wo_NAME="wo"

      integer :: wo_varid, dimids4(NDIMS4)
      ...
      character(len=*), parameter :: wo_UNITS="m/s"
      ...
      character(len=*), parameter :: wo_LNAME=&
      &"vertical velocity "
      ...
      retval = nf90_def_var(ncid, wo_NAME, NF90_REAL, dimids4, wo_varid)
            
      retval = nf90_put_att(ncid, wo_varid, UNITS, wo_UNITS)
      retval = nf90_put_att(ncid, wo_varid, LNAME, wo_LNAME)

      retval = nf90_put_att(ncid, wo_varid,'missing_value', missing_value)
      ...
      retval = nf90_put_var(ncid, wo_varid, wo)
